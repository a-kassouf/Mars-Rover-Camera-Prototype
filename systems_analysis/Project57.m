...
W_fold = W - LeafWidthFold/2;
%from 0.01 till W - x(7)
W_start = 0.01; %x of leaf-rigid connection
%W_start = LeafWidth/2;
...
nodes = [   W_start, 0, 0; %fix
            ...
            W_fold, 0, L;  
            W_fold, 0, foldedY;
            W_fold, zDir, foldedY+fixedY; %fix
            W, 0, L;
            0, 0, L;
            (W_fold - W_start)/2, 0, L;
            ]; 
...
%}
elements = [    1,   2; %leaf spring, width into screen
                2,   8; %rigidBody, platform 
                3,   4; %leaf spring fold p1, perp to the leaf spring
                4,   5; %leaf spring fold p2, chilling
                3,   6;
                2,   7;
                8,   3; %other half of the platform
                ];

nprops(1).fix = true;
nprops(5).fix = true;
nprops(8).mass = 2; %camera represented as a point mass
thicknessRb = 0.01;
